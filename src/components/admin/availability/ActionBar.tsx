...
export function ActionBar({
  ...
  return (
    <div className="sticky top-0 z-10 flex flex-col md:flex-row gap-4 items-center justify-between py-4 bg-[hsl(20,18%,5%)]/95 backdrop-blur-xl border-b border-white/10 mb-6 px-1">
      {/* Search Input */}
      <div className="relative w-full md:max-w-md">
        ...
        <input
          ...
          className={cn(
            "w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-10",
        ...
        {searchQuery.length > 0 && (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 rounded-full hover:bg-white/10 text-white/50 hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        )}
      ...
      <div className="flex items-center gap-3 w-full md:w-auto">
        <button
          ...
          className={cn(
            ...
            "bg-[#FBA919] text-zinc-900 font-bold px-5 py-2.5 rounded-xl text-sm",
        ...
        <AlertDialog.Root open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
          <AlertDialog.Trigger asChild>
            <button
              className={cn(
                "flex-1 md:flex-none flex items-center justify-center gap-2",
                "bg-red-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm",
            ...
            <AlertDialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-[24px] border border-white/10 bg-[hsl(20,18%,8%)] p-6 md:p-8 shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
              ...
              <div className="mt-8 gap-3 flex flex-col md:flex-row justify-end">
                <AlertDialog.Cancel asChild>
                  <button className="px-5 py-2.5 rounded-xl text-sm font-semibold border border-[#FBA919] text-[#FBA919] hover:bg-[#FBA919]/10 transition-colors">
                    Cancel
                  </button>
                </AlertDialog.Cancel>
                <AlertDialog.Action asChild>
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl text-sm font-semibold border border-transparent bg-red-500 text-white hover:bg-red-600 shadow-[0_0_16px_rgba(239,68,68,0.4)] transition-all"
                  >
                    Deselect All
                  </button>
                </AlertDialog.Action>
